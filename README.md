...
```sh
curl -fsSL --retry 3 --retry-delay 2 https://github.com/x-saola/litellm-ultis/releases/download/0.1.0/install.sh | sh
```
...
```sh
curl -fsSL --retry 3 --retry-delay 2 https://github.com/x-saola/litellm-ultis/releases/download/0.1.0/setup-telemetry.sh | bash
```
...
**bash / zsh:**
```sh
curl -fsSL --retry 3 --retry-delay 2 https://github.com/x-saola/litellm-ultis/releases/download/0.1.0/uninstall.sh | sh && unset ANTHROPIC_BASE_URL ANTHROPIC_AUTH_TOKEN && claude /logout && claude /login
```

**fish:**
```fish
curl -fsSL --retry 3 --retry-delay 2 https://github.com/x-saola/litellm-ultis/releases/download/0.1.0/uninstall.sh | sh; and set -e ANTHROPIC_BASE_URL; and set -e ANTHROPIC_AUTH_TOKEN; and claude /logout; and claude /login
```
